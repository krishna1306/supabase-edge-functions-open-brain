...
Replace the placeholder values with your actual credentials from the tracker.
...
Copy the **Function URL** from the output — it looks like `https://YOUR_PROJECT_REF.supabase.co/functions/v1/ingest-thought`. Paste it into your credential tracker.

---

## Step 8: Connect Slack to the Function

Now tell Slack where to send messages.

1. Go back to api.slack.com/apps → your Open Brain app
2. Left sidebar → **Event Subscriptions** → toggle **Enable Events ON**
3. Paste your Function URL into **Request URL**
4. Wait for the green "Verified ✓" checkmark (takes a few seconds)
5. Scroll to **Subscribe to bot events** → **Add Bot User Event**
6. Add both: `message.channels` and `message.groups`
7. Click **Save Changes** at the bottom
8. Slack will prompt you to reinstall — click **reinstall your app**

---

## Step 9: Test the Capture Flow

Go to your Slack capture channel and type:

> Reminder: follow up with Sarah about the API redesign timeline

Within a few seconds, you should get a threaded reply from your bot showing what was captured — type, topics, people, action items.

Check Supabase: **Table Editor → thoughts**. You should see one row with your message, a giant array of numbers (the embedding), and a metadata JSON object.

If nothing happens: check **Edge Functions → ingest-thought → Logs** for errors.
...
The MCP server is a second Edge Function that exposes five tools to any AI: search, list, stats, capture, and delete.
...
(The guide references code that should be pasted here, but it was truncated in the fetch. The actual implementation is what you already have in your project.)

### Set the Access Key

Pick a strong random string (or generate one with `openssl rand -hex 32`). This is your MCP authentication key.

```bash
supabase secrets set MCP_ACCESS_KEY=your-random-access-key-here
```

Paste the key into your credential tracker.
...
Copy the **Function URL** — it looks like `https://YOUR_PROJECT_REF.supabase.co/functions/v1/open-brain-mcp`. Paste it into your credential tracker.

---

## Step 11: Build Your MCP Connection URL

Combine your MCP Function URL with your access key:

```
https://YOUR_PROJECT_REF.supabase.co/functions/v1/open-brain-mcp?key=your-access-key-from-step-10
```

This is your **MCP Connection URL**. Copy it — you'll use it in Step 12.

---

## Step 12: Connect Your AI
...
2. Click **Add Connector** → **Add URL**
3. Paste your MCP Connection URL
4. Name it "Open Brain"
5. Click **Add**
6. Start a new conversation — click the "+" at the bottom → Connectors → toggle **Open Brain ON**

The tools appear automatically. Ask: "What thoughts do I have?" or "Search my brain for API redesign."

### ChatGPT Desktop

> ⚠️ Requires ChatGPT Plus or Pro. Developer Mode must be enabled.

1. Open ChatGPT Desktop → Settings → Apps & Connectors → Advanced settings
2. Toggle **Developer Mode ON**
3. Go to Apps & Connectors → Add Connector
4. Paste your MCP Connection URL
5. Name it "Open Brain"
6. Save

Start a conversation and ask it to search your brain. If it doesn't automatically use the tools, be explicit once: "Use the Open Brain search_thoughts tool to find my notes about [topic]." It learns the pattern quickly.

> 💡 If ChatGPT doesn't expose the tools even with Developer Mode on, it may not fully support remote MCP servers yet. Use the mcp-remote bridge (Option B below) as a fallback.

### Cline (VS Code Extension)

1. Open VS Code → Cline extension settings
2. Find **MCP Servers** section
3. Click **Edit MCP Settings JSON**
4. Add:
...
Replace `YOUR_PROJECT_REF` and `YOUR_ACCESS_KEY` with your actual values.

> 💡 If ChatGPT or another AI doesn't pick up the Open Brain tools automatically, prompt it explicitly the first time: "Use the Open Brain search_thoughts tool to search for [your query]." Once it uses the tool successfully in a conversation, it usually recognizes when to use it on its own. If it still doesn't use the brain on its own, be explicit: "Use the Open Brain search_thoughts tool to find my notes about project planning." After it gets the pattern once or twice in a conversation, it usually picks up the habit.
